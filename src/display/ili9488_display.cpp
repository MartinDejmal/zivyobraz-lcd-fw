...
bool Ili9488Display::begin(const DisplayConfig& cfg) {
  cfg_ = cfg;
  ...
  initialized_ = true;
  ZO_LOGI("ILI9488 initialized %ux%u (pins: mosi=%d sclk=%d cs=%d dc=%d rst=%d bl=%d)",
          cfg_.width, cfg_.height, cfg_.pins.mosi, cfg_.pins.sclk, cfg_.pins.cs, cfg_.pins.dc,
          cfg_.pins.rst, cfg_.pins.bl);
  return true;
}

void Ili9488Display::clear(uint16_t color565) {
  if (!initialized_) {
    return;
  }
...
void Ili9488Display::drawTestPattern() {
  if (!initialized_) {
    return;
  }
...
void Ili9488Display::drawStatusScreen(const StatusSnapshot& status) {
  if (!initialized_) {
    return;
  }
...
bool Ili9488Display::renderIndexed(const image::IndexedFramebuffer& framebuffer, int8_t rotate,
                                   bool partialRefresh) {
  if (!initialized_) {
    return false;
  }
...
void Ili9488Display::setBacklight(bool on) {
  if (cfg_.pins.bl < 0) {
    return;
  }
...
uint16_t Ili9488Display::mapColorIndex(uint8_t colorIndex) const {
  switch (colorIndex) {
    case 0:
      return TFT_WHITE;
    case 1:
...
bool Ili9488Display::mapCoordinates(uint16_t srcX, uint16_t srcY, int8_t rotate, uint16_t& outX,
                                    uint16_t& outY) const {
